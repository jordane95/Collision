...
public class CollisionOne2 : MonoBehaviour
{
    public float MoveSpeed;

    //speed用于保存碰撞前的速度，v用于计算碰撞后的速度
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        //获取碰撞到物体的Render组件并且更改其材质颜色
        collision.gameObject.GetComponent<Renderer>().material.color = Color.red;
        //计算碰撞后的速度
        v1 = (mass1 * speed1 + mass2 * speed2) / (mass1 + mass2);
        v2 = (mass1 * speed1 + mass2 * speed2) / (mass1 + mass2);
        Debug.Log(v1);
        Debug.Log(v2);
        if (this.name == "Cube1")
        {
